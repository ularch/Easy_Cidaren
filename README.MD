...
本隐私政策适用于您使用Easy_Cidaren（以下简称 “本工具”）过程中涉及的信息收集与处理行为，旨在保护您的个人信息安全与合法权益。
...
1. 不向任何第三方出售、共享或转让您的信息，法律法规要求除外；
2. 采取合理安全措施（如数据加密）保护信息，防止泄露、丢失或篡改；
3. 仅在实现上述目的所需的合理期限内存储信息，超出期限后将安全删除或匿名化处理。

**四、用户权利**

您有权查询、要求删除您的相关信息，如需行使该权利，请通过合理方式联系我们。

**五、政策更新**

本隐私政策可能根据业务调整更新，更新后将通过合理方式通知您，您继续使用本工具即视为接受更新后的政策。

若您使用本工具，即表示您已阅读、理解并同意本隐私政策的全部内容。
...
+ 自动模式：任务完成后按设置顺序自动开始下一个未完成任务，可设置等待秒数（默认2秒）
+ 顺序可选：降序/升序（默认降序）
+ 进度显示：增加“单元进度 + 总体进度”双进度条
+ 提示策略：开始任务确认默认自动同意；完成弹窗默认关闭；完成提示音默认关闭；错误提示音默认开启（均可在设置里改）
+ 启动行为：启动时自动打开 token 获取工具
+ 稳定性：题目缺少 `topic_mode` 时会重试；卡片数据异常时自动跳过该题并记录日志
