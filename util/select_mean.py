...
from api.main_api import query_word, CardDataError
...
def handle_query_word_mean(public_info) -> None:
    ...
    means = []
    options = public_info.word_query_result.get('options')
    means_payload = public_info.word_query_result.get('means')
    if not options and not means_payload:
        select_module.logger.warning("word_query_result missing options/means")
        raise CardDataError("word meanings missing")
    if options:
...
def word_examples(public_info, options) -> str:
    ...
    for word in options:
        if is_word_exist(public_info, word):
            means_payload = public_info.word_query_result.get('means')
            options_payload = public_info.word_query_result.get('options')
            if not means_payload and not options_payload:
                select_module.logger.warning("word_query_result missing options/means for examples")
                raise CardDataError("word examples missing")
            if means_payload:
                query_result = means_payload
